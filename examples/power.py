    ...
    for segment_list in voronoi_cell_map.values():
        for edge, (A, U, tmin, tmax) in segment_list:
            ...
            if True:
                ...
                if tmin is None:
                    tmin = -10; 
                edges.append((A + tmin * U, A + tmax * U))
                #edge_map[edge] = (A + tmin * U, A + tmax * U); print(str(edge)+' added: '+str(edge_map[edge])+' U={}'.format(U))
...
def display(S, R):
    ...
    for Si, Ri in zip(S, R):
        ax.add_artist(plot.Circle(Si, Ri, fill = False, alpha = 1, lw = 1,  color = '#8080f0', zorder = 1))
    ...
    line_list = LineCollection(edges, lw = 1., colors = 'k')#LineCollection(edge_map.values(), lw = 1., colors = 'k')
    line_list.set_zorder(0)
    ax.add_collection(line_list)
